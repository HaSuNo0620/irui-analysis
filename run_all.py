...
def stable_id(path, idx, meta):
    ncode=str((meta or {}).get("id","") or "").strip().upper() if isinstance(meta,dict) else ""
    if ncode:
        return ncode
...
def novel_body(text, meta):
    """Remove Syosetu711K's title/N-code/author/summary preamble when possible.

    meta.length corresponds to the source novel's published text length. The corpus
    prepends metadata/summary to `text`, so taking the final `length` characters is a
    robust way to isolate the novel body when the recorded length is plausible.
    """
    text=text or ""
    try:
        n=int((meta or {}).get("length",0))
    except Exception:
        n=0
    if 0 < n <= len(text):
        return text[-n:]
...
def main():
    ...
    ap.add_argument("--input",required=True,help="Strictly preselected work-level JSONL glob")
    ...
    for path,idx,obj in iter_jsonl(paths):
        raw_text=obj.get("text","") or ""; meta=obj.get("meta",{}) or {}
        body=novel_body(raw_text,meta)
        if not body.strip(): continue
        meta_text=" ".join(flatten_strings(meta)); head=raw_text[:int(cfg["head_chars"])]
        nh,rel=lexical_hits(meta_text+"\n"+head)
        cid=stable_id(path,idx,meta)
        candidates.append({
            "candidate_id":cid,"ncode":str(meta.get("id","") or ""),
            "source_file":path,"row_idx":idx,
            "title":pick_meta(meta,["title","name"]),
            "author":pick_meta(meta,["author"]),
            "userid":pick_meta(meta,["userid"]),
            "chapters":pick_meta(meta,["chapters"]),
            "genre":pick_meta(meta,["genre"]),
            "biggenre":pick_meta(meta,["biggenre"]),
            "points":pick_meta(meta,["points"]),
            ...
            "raw_text_chars":len(raw_text),"body_text_chars":len(body),
            "meta_json":json.dumps(meta,ensure_ascii=False)
        })
        texts[cid]=body
    cdf=pd.DataFrame(candidates); cdf.to_csv(out/"candidates.csv",index=False)
    if cdf.empty: raise SystemExit("No candidate works were present in the preselected input.")
    ...
    for row in candidates:
        ...
        for j,(st,en,raw) in enumerate(chunks):
            manifest.append({
                "candidate_id":cid,"ncode":row.get("ncode",""),"title":row.get("title",""),
                "author":row.get("author",""),"chapters":row.get("chapters",""),
                "date_raw":row.get("date_raw",""),"chunk_idx":j,
                "start_char":st,"end_char":en,
    ...
    assign=mdf[["candidate_id","ncode","chunk_idx","position"]].copy()
    ...
    print(f"Candidate works: {len(cdf)}")
    print(f"Unique N-codes: {cdf.ncode.nunique()}")
    print(f"Full-text body chunks: {len(mdf)}")
    ...
    print("Selection evidence is used only for work membership; downstream analysis uses novel bodies.")
